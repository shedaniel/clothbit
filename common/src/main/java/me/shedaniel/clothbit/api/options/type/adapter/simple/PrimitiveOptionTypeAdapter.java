...
public class PrimitiveOptionTypeAdapter<T> implements OptionTypeAdapter {
    private OptionTypeConstructor<T> constructor;
    private Set<Type> types;
    
    public PrimitiveOptionTypeAdapter(OptionTypeConstructor<T> constructor, Type... types) {
    ...
    public <R> Optional<OptionType<? extends R>> forType(TypeToken<R> typeToken, OptionTypesContext ctx) {
        if (types.contains(typeToken.getType())) {
            return Optional.of((OptionType<? extends R>) constructor.construct(typeToken.getRawType().isPrimitive()));
        }
    ...
    public interface OptionTypeConstructor<T> {
        OptionType<T> construct(boolean primitive);
    }
}
